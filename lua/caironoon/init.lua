...
function M.colorscheme()
    ...
    elseif vim.g.caironoon_config.style == 'light' then
...
---Toggle between caironoon styles
function M.toggle()
    local index = vim.g.caironoon_config.toggle_style_index + 1
    if index > #vim.g.caironoon_config.toggle_style_list then index = 1 end
    M.set_options('style', vim.g.caironoon_config.toggle_style_list[index])
    M.set_options('toggle_style_index', index)
    if vim.g.caironoon_config.style == 'light' then
...
function M.setup(opts)
    ...
    if vim.g.caironoon_config.toggle_style_key then
      vim.api.nvim_set_keymap('n', vim.g.caironoon_config.toggle_style_key, '<cmd>lua require("caironoon").toggle()<cr>', { noremap = true, silent = true })
    end
end
